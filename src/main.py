...
with u.connect() as conn:
  with conn.cursor() as cursor:
    ...
    users_data = [(fake.name(), fake.unique.email(), fake.password()) for _ in range(AMOUNT_OF_USERS)]
    ...
    insert_query = 'INSERT INTO comments (content, created_at, user_id, post_id) VALUES (%s, %s, %s, %s)'
    comments_data = [(fake.text(), fake.date_time(), fake.random_int(1, AMOUNT_OF_USERS), fake.random_int(1, AMOUNT_OF_POSTS)) for _ in range(AMOUNT_OF_COMMENTS)]
